...
import 'package:babigo/modules/trip_module/widgets/trip_card.dart';
import 'package:babigo/modules/booking_module/widgets/search_filter_bar.dart';
import 'package:babigo/modules/booking_module/widgets/filter_criteria.dart';
import 'package:babigo/modules/booking_module/screens/passenger/reservation_success_screen.dart';

enum SortMode { none, priceAsc, ratingDesc, timeAsc }

class AdminSearchResultScreen extends StatefulWidget {
  final String depart;
  final String destination;
  final String? date; // optionnelle
  final VoidCallback onBack;

  const AdminSearchResultScreen({
    super.key,
  ...
  @override
  State<AdminSearchResultScreen> createState() => _AdminSearchResultScreenState();
}

class _AdminSearchResultScreenState extends State<AdminSearchResultScreen> {
  ...
  Future<void> _reserveTripAndCreateRecord(
    ...
    await db.runTransaction((newBooking) async {

      // 1) Relire l’annonce
      final snap = await newBooking.get(announceRef);
      ...
      newBooking.update(announceRef, {'reservedSeats': reserved + 1});
