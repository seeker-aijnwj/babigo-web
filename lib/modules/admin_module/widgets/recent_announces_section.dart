...
import '../../../app/widgets/txt_components.dart';
import '../../../app/core/utils/colors.dart';

// Voir plus -> historique complet
import '../../admin_module/database/models/admin/utilisateur.dart';
import '../../booking_module/screens/driver/driver_history_screen.dart';
// Tap sur une bulle -> détail
import '../../booking_module/screens/driver/driver_announce_detail_screen.dart';

class RecentAnnouncesSection extends StatelessWidget {

  /// Nombre d’éléments max affichés ET hauteur figée (5 “slots”).
  final int limit;

  final Utilisateur? selectedUser;

  const RecentAnnouncesSection({super.key, this.limit = 5, this.selectedUser});

  @override
  Widget build(BuildContext context) {
    final uid = (selectedUser != null) ?
         selectedUser?.id :
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        final announces = <Map<String, String>>[];
        for (final doc in docs) {
          ...
          announces.add({'id': doc.id, 'from': from, 'to': to});

          if (announces.length >= limit) break;

        }

        return _buildContainer(context, announces, forceFixedHeight: true);
      },
    );
  }
...
class _AnimatedCarIconState extends State<_AnimatedCarIcon>
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      child: Icon(Icons.directions_car, color: AppColors.secondColor, size: 20),
    );
  }
}
