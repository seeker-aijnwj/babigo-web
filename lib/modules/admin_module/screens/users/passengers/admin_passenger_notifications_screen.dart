...
import 'package:babigo/app/core/utils/colors.dart'; // pour backgroundColor / mainColor si tu les as

class AdminPassengerNotificationsScreen extends StatefulWidget {

  final Utilisateur? selectedUser;

  const AdminPassengerNotificationsScreen({
    super.key,
    this.selectedUser
  });

  @override
  State<AdminPassengerNotificationsScreen> createState() => _AdminPassengerNotificationsScreenState();
}

class _AdminPassengerNotificationsScreenState extends State<AdminPassengerNotificationsScreen> {
  ...
  void initState() {
    super.initState();
    _uid = (widget.selectedUser != null)
        ? widget.selectedUser!.id : FirebaseAuth.instance.currentUser?.uid;
  }

  Query<Map<String, dynamic>> _query() {
    // Notifications triées de la plus récente à la plus ancienne
    return _fs.collection('users/${_uid!}/notifications')
  ...
  Future<void> _markAllAsRead() async {
    ...
    final q1 = await _fs.collection('users/${_uid!}/notifications')
        .where('isRead', isEqualTo: false)
        .get();

    final q2 = await _fs.collection('users/${_uid!}/notifications')
